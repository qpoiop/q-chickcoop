...
export class Panels {
  ...
  _renderInventory() {
    ...
    const resources = [
      { k: 'Scrap', v: s.gold, c: '#ffd23f' }, { k: 'Skill Points', v: s.skillPoints, c: '#59ff9d' },
      { k: 'Level', v: s.level, c: '#7ff2e8' }, { k: 'Kills', v: s.kills, c: '#dfeef6' },
    ];
    ...
    const wepHTML = Object.entries(WEAPONS).map(([k, w]) => {
      ...
      const status = eq ? 'Equipped' : (own ? 'Owned' : 'Locked · ' + (w.cost || 0) + ' scrap');
      ...
      return `<div style="display:flex;align-items:center;gap:11px;opacity:${own ? 1 : 0.45}">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-family:'Chakra Petch';font-weight:600;font-size:14px;color:#eef6fb">${w.name}</div>
          <div style="font-size:11px;letter-spacing:2px;color:${statusColor};text-transform:uppercase">${status}</div>
        </div></div>`;
    }).join('');

    const aug = [];
    BRANCHES.forEach((b) => b.nodes.forEach((n) => { const r = (s.ranks || {})[n.id] || 0; if (r > 0) aug.push({ name: n.name, rank: r, max: n.max, color: b.color }); }));
    const augCount = aug.reduce((a, x) => a + x.rank, 0) + ' pts spent';
    const augBody = aug.length
      ...
      : `<div style="font-size:13px;color:#5f7486;line-height:1.5">No augments yet. Earn skill points by leveling, then spend them in the Tech Tree.</div>`;

    const div = this._mount(this._header('Field Loadout', 'INVENTORY') + `
      <div style="flex:1;overflow:auto;display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px;align-content:start">
        <div class="card"><div class="card-title">Resources</div><div style="display:flex;flex-direction:column;gap:10px">${resHTML}</div></div>
        <div class="card"><div class="card-title">Weapon Frames</div><div style="display:flex;flex-direction:column;gap:9px">${wepHTML}</div></div>
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:baseline;padding-bottom:10px;border-bottom:1px solid #223140;margin-bottom:12px">
            <span style="font-family:'Chakra Petch';font-weight:600;font-size:14px;letter-spacing:2px;color:#dfeef6;text-transform:uppercase">Active Augments</span>
      ...
      <div style="margin-top:14px;display:flex;gap:12px;padding-top:14px;border-top:1px solid #223140">
        <button class="hud-btn" data-goto="weapons" style="clip-path:none">Open Arsenal</button>
        <button class="hud-btn" data-goto="skills" style="clip-path:none">Tech Tree</button>
      </div>`);
  ...
  _renderArsenal() {
    ...
    const cards = Object.entries(WEAPONS).map(([k, w]) => {
      ...
      let tag = '';
      if (eq) tag = `<span style="font-size:11px;letter-spacing:2px;color:#59ff9d;text-transform:uppercase;border:1px solid #2a5a44;padding:3px 8px">Equipped</span>`;
      ...
      else tag = `<span style="font-size:11px;letter-spacing:2px;color:#7f94a6;text-transform:uppercase;border:1px solid #2a3a4a;padding:3px 8px">Owned</span>`;
      const bars = [{ k: 'Rate', pct: Math.round(w.bars.rate * 100) }, { k: 'Damage', pct: Math.round(w.bars.dmg * 100) }, { k: 'Area', pct: Math.round(w.bars.area * 100) }];
      ...
      return `<button data-pick="${k}" style="text-align:left;font-family:'Rajdhani';cursor:${interactive ? 'pointer' : 'default'};background:${bg};border:1.5px solid ${border};padding:16px;opacity:${interactive ? 1 : 0.6};transition:border-color .15s" ${interactive ? `data-hover="${w.color}" data-base="${border}"` : ''}>
        ...
        <div style="font-family:'Chakra Petch';font-weight:700;font-size:19px;color:#eef6fb;margin-top:12px;letter-spacing:1px">${w.name}</div>
        <div style="font-size:13px;line-height:1.4;color:#9fb1c0;margin-top:4px">${w.desc}</div>
        <div style="margin-top:12px;display:flex;flex-direction:column;gap:6px">${barsHTML}</div>
      </button>`;
    }).join('');

    const div = this._mount(this._header('Weapon Frames', 'ARSENAL') + `
    ...
    div.querySelectorAll('[data-pick]').forEach((b) => {
      b.onclick = () => this.g.pickWeapon(b.dataset.pick);
      this._hoverBorder(b);
    });
  }
  ...
  _renderSkills() {
    ...
    const cols = BRANCHES.map((b) => {
      const nodes = b.nodes.map((n, i) => {
        ...
        const costTxt = maxed ? 'MAXED' : (prevOk ? n.cost + ' PT' : 'LOCKED');
        ...
        return `<button ${afford ? `data-buy="${n.id}"` : ''} style="text-align:left;font-family:'Rajdhani';cursor:${afford ? 'pointer' : 'default'};background:${bg};border:1px solid ${border};padding:11px 13px;opacity:${prevOk ? 1 : 0.5};transition:border-color .15s,background .15s" ${afford ? `data-hover="#7ff2e8" data-base="${border}"` : ''}>
          <div style="display:flex;justify-content:space-between;align-items:center;gap:6px">
            <span style="font-family:'Chakra Petch';font-weight:600;font-size:14px;color:#eef6fb;letter-spacing:.5px">${n.name}</span>
            <span style="font-family:'Chakra Petch';font-size:12px;color:${rankColor};white-space:nowrap">${cur}/${n.max}</span>
          </div>
          <div style="font-size:12px;line-height:1.35;color:#9fb1c0;margin-top:3px">${n.desc}</div>
      ...
      return `<div style="display:flex;flex-direction:column;gap:10px;min-width:0">
        <div style="display:flex;align-items:center;gap:8px;padding-bottom:8px;border-bottom:1px solid #223140">
          ...
          <span style="font-family:'Chakra Petch';font-weight:600;font-size:15px;letter-spacing:2px;color:#dfeef6;text-transform:uppercase">${b.name}</span>
        </div>${nodes}</div>`;
    }).join('');

    const statLine = [
      { k: 'DMG', v: '×' + md.dmg.toFixed(2) }, { k: 'Rate', v: '×' + md.rate.toFixed(2) }, { k: 'Move', v: '×' + md.spd.toFixed(2) },
      { k: 'Max Hull', v: s.maxHp + Math.round(md.hp) }, { k: 'Armor', v: Math.round(md.armor * 100) + '%' }, { k: 'Crit', v: Math.round(md.crit * 100) + '%' },
      { k: 'Dash', v: md.dashchg + ' chg' }, { k: 'XP', v: '×' + md.xp.toFixed(2) }, { k: 'Scrap', v: '×' + md.gold.toFixed(2) },
    ...
    const right = `<div style="text-align:right"><div style="font-family:'Chakra Petch';font-size:30px;font-weight:700;color:#59ff9d;line-height:1">${s.skillPoints}</div><div style="font-size:11px;letter-spacing:2px;color:#7f94a6;text-transform:uppercase">Points</div></div>`;
    const div = this._mount(this._header('Neural Tech Tree', 'AUGMENT LOADOUT', right) + `
