...
- [~] **파란 void로 걸어나감** — (개선됨, 완전치 않음) 파란 = 콘텐츠 밖으로 뻗은
  ...
  - 숲 4방향 봉쇄 확인(W-16/E-8/N-22/S+7). **시티도 trimOpen 0.9 적용** — 동쪽
    VOID walk-off(x=51) 수정, 앵커 전부 도달, 파편화 없음(729/734).
  - **잔여**: 폭포(boss)는 밀도 트림이 안 맞음 — `Town_plane`이 주 플레이 면이라
    트림하면 extraction 도달 불가(소프트락) + 봉쇄 불완전. **다른 접근 필요**
    (예: 콘텐츠 bbox 기반 경계, 또는 도달-성분 + 반경 제한).
