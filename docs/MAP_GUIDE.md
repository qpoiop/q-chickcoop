...
| city | 없음(절차 레인) | - | 첫 전투맵 | 방향성 레인 34×62, 끝에서 시작→전진 |
| boss | 없음(절차 콜로세움) | - | 보스 아레나 | 평면·walkFrac 1.0, 보스 중앙·바이올렛 |
| ~~main~~ | ~~mystical_forest~~ | - | 제거됨 | 완전 삭제 |

**전 맵 절차적(model:null) — scene GLB 없음.** 데코 GLB는 플레이 아레나로 부적합
(오클루전·하버스트 콜리전이 이동 막음·배치난망) — §4 교훈.
**전투 맵은 방향성으로.** 정중앙 대칭 아레나=진행감 없음(유저 지적). 시티는 근처
가장자리 스폰→앞으로 전진(코어 중간·깊숙이, 포탈=먼 끝). 몹은 **플레이어 주변 링
스폰**(config.spawn.ring, 진행방향 편향) + **스폰 즉시 aggro** → 전진=전투(고정 코너
스폰은 지나쳐도 안 싸움). `_enemySpawnPoint`.
